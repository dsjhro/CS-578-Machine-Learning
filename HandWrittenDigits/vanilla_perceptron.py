...
def sgn(x,w):
    ...
    '''
    Takes input vecotr x, and weight vecotr y and computes the dot product
...
def instPerceptron(instance, weight, y, rate):
    ...
    if yprime != y:
        ...
        if y == 0: # Changed yprime to y check results
...
def digGuess(instance, label, weight, learningRate):
    ...
    for digit in range(0,10):
        ...
        weightSum[digit], weight[digit] = instPerceptron(instance, weight[digit], y_gold[digit],learningRate)
    
    return(weightSum.argmax(), weight)
    
def vanillaPerceptronTrain(data,label,weight, learningRate):
    
    guesses = []
    for entry in range(0,len(data)):
        
        guessIndex,weight = digGuess(data[entry], label[entry], weight, learningRate)
        guesses.append(guessIndex)
        
    return(guesses, weight)


def vanillaPerceptronTest(data,label,weight, learningRate):
    
    guesses = []
    for entry in range(0,len(data)):
        
        guessIndex,weight = digGuess(data[entry], label[entry], weight, 0)
        guesses.append(guessIndex)
        ...
        if entry == 'trainingSet':
            ...
            for i in trainingSize:
                ...
                for runs in range(0,epochs):
                    ...
                    guessesTR1,weights = vanillaPerceptronTrain(data, labels, weights, learningRate)
                    
                guessesTE1,weights = vanillaPerceptronTest(procTestData, testLabels, weights, 0)
        ...
        if entry == 'epochs':
            ...
            for i in epochs:
                ...
                for runs in range(0,i):
                    ...
                    guessesTR2,weights = vanillaPerceptronTrain(data, labels, weights, learningRate)
                
                guessesTE2,weights = vanillaPerceptronTest(procTestData, testLabels, weights, 0)
        ...
        if entry == 'learningRate':
            ...
            for i in learningRate:
                ...
                for runs in range(0,epochs):
                    ...
                    guessesTR3,weights = vanillaPerceptronTrain(data, labels, weights, i)
                    
                guessesTE3,weights = vanillaPerceptronTest(procTestData, testLabels, weights, 0)
...
if __name__ == "__main__":
    ...
    for x in range(0,epochs):
        ...
        guessesTR,weights = vanillaPerceptronTrain(procTrainData, trainLabels, weights, learningRate)
    
    # Uses the weight from the training process to make guesses for testing
    guessesTE,weights = vanillaPerceptronTest(procTestData, testLabels, weights, 0)
